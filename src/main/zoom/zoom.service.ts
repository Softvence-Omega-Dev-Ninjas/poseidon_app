...
export class ZoomService {
  ...
  async createMeetingForBasicUser(providerId: string, dto: CreateMeetingDto) {
    ...
    if (!zoomAccount) {
      throw new HttpException(
        'Zoom account not found for provider',
    ...
    try {
      ...
      const meetingResponse = await axios.post(
        `${apiBase}/users/${zoomAccount.zoomUserId}/meetings`,
        {
          ...
          duration: Math.min(dto.duration, 40),
          agenda: dto.agenda || '',
          settings: {
            host_video: true,
      ...
      return {
        meeting,
        message:
          'Meeting created successfully for Basic user. Note: max 40 min duration for group meetings.',
      };
    } catch (error: any) {
      throw new HttpException(
        error.response?.data || error.message,
